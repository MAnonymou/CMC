...
from moduleqap.qat_model import QATQuantSuperModel
import torch.backends.cudnn as cudnn
from core.config import cfg
from .utilsqap import Adaptive_BN
import datetime
import time
...
def evaluation_model(model, test_loader):
    ...
    for cur_iter, (inputs, labels) in enumerate(test_loader):
        ...
        if i%10==0:
            ...
            timestamp = datetime.datetime.strftime(curr_t, '%Y-%m-%d %H:%M:%S')
            logger.info('{}: Evaluation finished {:.2f}%'.format(timestamp,i/len(test_loader)*100))
        i+=1
